<template>
    <app-layout title="Monitoring">
        ...
        <div class="fixed bottom-0 md:inset-y-0 right-0 p-8 w-full md:w-40 flex z-50">
            ...
            <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-xl">
                    ...
                    <div class="grid grid-cols-2">
                         <div v-if="datas.category != null && datas.category != undefined" class="border-gray-100 flex flex-nowrap p-7"
                            ...
                            <p class="self-center text-base md:text-xl text-gray-700 inline-block ml-4">{{ datas.category.name }}</p>
                        </div>
                        <div v-if="datas.object != null && datas.object != undefined" class="col-span-2 md:col-span-1 p-7 flex flex-nowrap">
                            ...
                            <p class="self-center text-base md:text-xl text-gray-700 inline-block ml-4">{{ datas.object.name }}</p>
                        </div>
                    </div>
                            ...
                            <div class="grid grid-cols-4 md:grid-cols-3 gap-6 mt-3">
                                <div class="col-span-4 md:col-span-3" v-for="(item, index) in form_inputs" :key="`input-${index}`">
                                    <div v-if="item.mode == 'input'">
                                        <div @focus="focusIn(index)" @blur="focusOut(), item.option = false" :class="{'border-t-4 border-b-0 border-purple-400 pb-3': focus == index, 'border-y-2 border-gray-100': focus != index}" tabindex="-1" class="rounded-lg bg-white">
                                            <div class="grid grid-cols-2 p-3 border-b-2 border-gray-100">
                                                <div class="self-center col-span-2 md:col-span-1 flex justify-self-start flex space-x-2 relative">
                                                    <button @click="item.option = !item.option" @focus="focusIn(index)" @blur="focusOut()" type="button" class="text-gray-900 bg-white text-center focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-2 py-2">
                                                        <more-vertical-icon size="20" class="inline-block"/>
                                                    </button>
                                                    <div v-if="item.option" class="shadow absolute top-10 left-0 bg-white rounded overflow-hidden text-sm">
                                                    ...
                                                    <button @click="onDeleteInput(index)" @focus="focusIn(index)" @blur="focusOut()" type="button" class="text-gray-900 bg-white text-center focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-2 py-2">
                                                ...
                                                <div class="w-full relative col-span-2 md:col-span-1 self-end" @focus="focusIn(index)" @blur="focusOut()">
                                                    ...
                                                    <button @click="focus_dropdown = index" @focus="focusIn(index)" @blur="focusOut()" type="button" class="text-gray-900 w-full bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-5 py-2.5 mr-2">
                                            ...
                                            <div class="my-4 px-4">
                                                <div class="my-3" v-if="item.preview != null && item.image != null">
                                                    ...
                                                    <div class="grid grid-cols-2 mb-3">
                                                        ...
                                                        <div role="button" @click="item.image = null, item.preview = null" class="text-red-600 text-right">
                                                            <p>Hapus gambar</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div>
                                                    ...
                                                    <div class="flex gap-x-4">
                                                        <input @focus="focusIn(index)" @blur="focusOut()" placeholder="e.g Kondisi Penambat" type="text" v-model="item.label" id="base-input" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-purple-500 block w-full p-2.5">
                                                ...
                                                <div class="mt-3 p-3 border-2 rounded-lg" v-if="checkMultiInput(item.type_input)">
                                                    ...
                                                    <div v-if="item.options.length > 0">
                                                        <div class="flex my-2" v-for="(option, indexOption) in item.options" :key="`option-${indexOption}`">
                                                            <input @focus="focusIn(index)" @blur="focusOut()" :placeholder="`Opsi ${indexOption + 1}`" type="text" v-model="option.value" :id="`option-${index}-${indexOption}-input`" class="bg-gray-50 w-1/2 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-purple-500 block w-full p-2.5">
                                                            <button @click="removeOption(index, indexOption)" @focus="focusIn(index)" @blur="focusOut()" type="button" class="text-gray-900 bg-white text-center focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-2 py-2">
                                                                <trash-icon size="20" class="inline-block"/>
                                                            </button>
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="mt-3" v-if="item.description != null">
                                                    ...
                                                    <input @focus="focusIn(index)" @blur="focusOut()" placeholder="Masukkan deskripsi input" type="text" v-model="item.description" id="description-input" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-purple-500 block w-full p-2.5">
                                                </div>
                                                <div class="mt-3" v-if="item.placeholder != null">
                                                    ...
                                                    <input @focus="focusIn(index)" @blur="focusOut()" placeholder="Masukkan sugesti input" type="text" v-model="item.placeholder" id="placeholder-input" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-purple-500 block w-full p-2.5">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div @focus="focusIn(index)" @blur="focusOut(), item.option = false" :class="{'border-t-4 border-b-0 border-purple-400 pb-3': focus == index, 'border-y-2 border-gray-100': focus != index}" tabindex="-1" class="rounded-lg bg-white">
                                            ...
                                            <div class="my-4 px-4">
                                                ...
                                                <div>
                                                    ...
                                                    <label for="base-input" class="block mb-2 text-sm font-medium text-gray-900">Subjek Gambar</label>
                                                    <div class="relative">
                                                        <input @focus="focusIn(index)" @blur="focusOut()" placeholder="e.g Gambar Penambat" type="text" v-model="item.label" id="base-input" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-purple-500 block w-full p-2.5">
                                                    </div>
                                                </div>
                                                <div class="mt-3" v-if="item.description != null">
                                                    <label for="base-input" class="block mb-2 text-sm font-medium text-gray-900">Deskripsi</label>
                                    ...
                                    <div v-if="item.mode == 'description'">
                                        <div @focus="focusIn(index)" @blur="focusOut(), item.option = false" :class="{'border-t-4 border-b-0 border-purple-400 pb-3': focus == index, 'border-y-2 border-gray-100': focus != index}" tabindex="-1" class="rounded-lg bg-white">
                                            ...
                                            <div class="my-4 px-4">
                                                <div>
                                                    <label for="base-input" class="block mb-2 text-sm font-medium text-gray-900">Subjek</label>
                                                    <div class="relative">
                                                        <input @focus="focusIn(index)" @blur="focusOut()" placeholder="e.g Subjek Deskripsi" type="text" v-model="item.label" id="base-input" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-purple-500 block w-full p-2.5">
                                                    </div>
                                                </div>
                                                <div class="mt-3">
                                                    <label for="base-input" class="block mb-2 text-sm font-medium text-gray-900">Deskripsi</label>
                                            ...
                                            <div class="my-4 px-4">
                                                <div>
                                                    ...
                                                    <div class="relative mt-3">
                                                        <label for="base-input" class="block mb-2 text-sm font-medium text-gray-900">Subjek</label>
                                                        <input @focus="focusIn(index)" @blur="focusOut()" placeholder="e.g Subjek" type="text" v-model="item.label" id="base-input" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-purple-500 block w-full p-2.5">
                                                    </div>
                                                </div>
                                                <div class="mt-3" v-if="item.description != null">
                                                    <label for="base-input" class="block mb-2 text-sm font-medium text-gray-900">Deskripsi</label>
...
export default defineComponent({
    ...
    methods: {
        ...
        removeOption(index, index_option) {
        ...
        setTypeInputDropdown(index, type){
            let multiInput = ['dropdown', 'checkbox', 'radio']
            this.form_inputs[index].options = []
